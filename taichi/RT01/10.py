...
def PBR(ray, record, normal: vec3) -> Ray:
    roughness = record.obj.mtl.roughness
    metallic = record.obj.mtl.metallic
    transmission = record.obj.mtl.transmission
    ior = record.obj.mtl.ior

    V = -ray.direction
    N = TBN(record.obj.mtl.normal) @ normal
    ...
    if ti.random() < transmission:  # 折射部分
        ...
        if ti.random() < F + metallic and outer > 0:
            ...
            ray.direction = refract(ray.direction, N, eta)
    else:
