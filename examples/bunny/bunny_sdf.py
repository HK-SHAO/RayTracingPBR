...
def sd_bunny(p: vec3) -> float:
    ...
    if length(p) > 1.0:
        ...
        f10=sin(mat4(-.34,.06,-.59,-.76,.10,-.19,-.12,.44,.64,-.02,-.26,.15,-.16,.21,.91,.15)@f00+
        ...
        f11=sin(mat4(-1.11,.55,-.12,-1.00,.16,.15,-.30,.31,-.01,.01,.31,-.42,-.29,.38,-.04,.71)@f00+
        ...
        f12=sin(mat4(.44,-.06,-.79,-.46,.05,-.60,.30,.36,.35,.12,.02,.12,.40,-.26,.63,-.21)@f00+
        ...
        f13=sin(mat4(-.41,-.24,-.71,-.25,-.24,-.75,-.09,.02,-.27,-.42,.02,.03,-.01,.51,-.12,-1.24)@f00+
...
WORLD_LIST = [
    SDFObject(type=SHAPE_BUNNY,
                transform=Transform(vec3(0, 0, 0), vec3(0, 90, 90), vec3(1, 1, 1)),
...
while window.running:
    ...
    times += 1
    if times > 100:
        window.save_image('sdf_bunny' + '.out.png')
        break
    window.show()