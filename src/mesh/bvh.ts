...
export function buildAabbBvh(
  ...
  const build = items.map((item) => ({
    ...item,
    centre: [...item.centre] as [number, number, number],
  }));
  const nodes: BvhNode[] = [];
  if (build.length === 0) {
    return [{ min: [0, 0, 0], max: [0, 0, 0], start: 0, count: 0 }];
  }
  const split = (nodeIndex: number, start: number, count: number): void => {
    const { min, max } = bounds(build, start, count);
    if (count <= 1) {
      nodes[nodeIndex] = { min, max, start: build[start]?.index ?? 0, count: 1 };
      return;
    }
    const size = [max[0] - min[0], max[1] - min[1], max[2] - min[2]] as const;
    const axis = size[0] >= size[1] && size[0] >= size[2] ? 0 : size[1] >= size[2] ? 1 : 2;
    const slice = build.slice(start, start + count);
    slice.sort((a, b) => (a.centre[axis] ?? 0) - (b.centre[axis] ?? 0));
    for (let i = 0; i < slice.length; i++) build[start + i] = slice[i]!;
    const mid = count >> 1;
    if (mid === 0 || mid === count) {
      nodes[nodeIndex] = { min, max, start: build[start]?.index ?? 0, count: 1 };
      return;
    ...
    nodes.push({ min: [0, 0, 0], max: [0, 0, 0], start: 0, count: 0 });
    nodes[nodeIndex] = { min, max, start: left, count: 0 };
    split(left, start, mid);
    split(right, start + mid, count - mid);
  };
  nodes.push({ min: [0, 0, 0], max: [0, 0, 0], start: 0, count: 0 });
  split(0, 0, build.length);
  return nodes;
}
